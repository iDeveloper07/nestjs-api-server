...
describe('AppController (e2e)', () => {
  ...
  it('/api/users/:userId (GET)', async () => {
    ...
    jest.spyOn(global, 'fetch').mockImplementation(() =>
      Promise.resolve({
        json : () => Promise.resolve(mockExternalUserData),
      } as any)
    );
    ...
    expect(response.body.data).toEqual(mockExternalUserData.data);
  }, actionTimeLimit);


  it('/api/users/:userId/avatar (GET)', async () => {
    ...
    const mockAvatarBuffer = Buffer.from('mockavatar');

    jest.spyOn(fs, 'writeFileSync').mockImplementation(() => undefined);
    jest.spyOn(fs, 'readFileSync').mockImplementation(() => mockAvatarBuffer);
    jest.spyOn(crypto, 'createHash').mockReturnValue({
      update : () => ({ digest: () => 'dummyhash' }),
    } as any);
    ...
    expect(response.text).toBe(base64Avatar);
  }, actionTimeLimit);

  it('/api/users/:userId/avatar (DELETE)', async () => {
